...
_产品_ 通过 _SKU_（读作“skew”，是库存管理单元的缩写）进行标识。_客户_ 会下达 _订单_ 。一个订单通过一个 _订单引用_ 来标识，
并包含多个 _订单项_ ，每个订单项都有一个 _SKU_和_数量_ 。例如：
...
采购部门会订购小的 _批次_ 库存。一个 _批次_ 库存具备一个名为 _引用_ 的唯一 ID、一个 _SKU_ 和一个 _数量_。
...
我们需要将 _订单项_ 分配（_allocate_）到 _批次_ 。当我们将某条订单项分配到某个批次时，我们会从该特定批次发送库存到客户的配送地址。
当我们将 _x_ 单位的库存分配到一个批次时，该批次的 _可用数量_ 会减少 _x_。例如：

- We have a batch of 20 SMALL-TABLE, and we allocate an order line for 2 SMALL-TABLE.
我们有一个包含 20 个 SMALL-TABLE 的批次，并分配了一个包含 2 个 SMALL-TABLE 的订单项。
- The batch should have 18 SMALL-TABLE remaining.
该批次应剩余 18 个 SMALL-TABLE。
...
我们有一个包含 1 个 BLUE-CUSHION 的批次，以及一个包含 2 个 BLUE-CUSHION 的订单项。
...
我们有一个包含 10 个 BLUE-VASE 的批次，并分配了一个包含 2 个 BLUE-VASE 的订单项。
...
批次如果当前正在运输，则有一个 _ETA_ （预计到达时间），否则可能在 _仓库库存_ 中。我们优先将订单分配给仓库库存，而不是运输批次。对于运输批次，我们按预计到达时间最早的顺序进行分配。
****
